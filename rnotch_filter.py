...
def rnotch_filter(shape, D0=0, angle=0, ftype='ideal', reject=True, W=1, n=1):
    ...
    for k in range(K):
        ...
        if (ftype == 'ideal'):
            ...
            Hk[:, 0:center - D0[k] + 1] = np.exp(-R ** 2 / (2 * width ** 2))
            Hk[:, center + D0[k]:] = np.exp(-R ** 2 / (2 * width ** 2))
            Hk = 1 - Hk
            #afmetingen van de notch in het midden bepalen
            D0_center = 100
            #juiste D0 berekenen
            while (np.exp(-(center-D0[k]) ** 2 / (2 * D0_center ** 2)) < 0.01):
                D0_center += 0.1
            while (np.exp(-(center-D0[k]) ** 2 / (2 * D0_center ** 2)) > 0.01):
                D0_center -= 0.1
            #breedte van de notch berekenen
            for i in range(center, len(Hk[0])):
                if (Hk[i, 0] > 0.9):
                    breedte = i - center
                    break
            Hk2[center - breedte:center + breedte, :] = np.exp(-C ** 2 / (2 * D0[k] ** 2))
            Hk[center - breedte:center + breedte, center - D0[k]:center + D0[k]] = Hk2[center - breedte:center + breedte,center - D0[k]:center + D0[k]]

        elif (ftype == 'butterworth'):
            Hk[:, 0:center - D0[k] + 1] = 1 / (1 + (R / D0[k]) ** (2 * n))
            Hk[:, center + D0[k]:] = 1 / (1 + (R / D0[k]) ** (2 * n))
            Hk = 1 - Hk
            # afmetingen van de notch in het midden bepalen
            D0_center = 100
            # juiste D0 berekenen
            while (1 / (1 + ((center-D0[k]) / D0_center) ** (2 * n)) < 0.01):
                D0_center += 0.1
            while (1 / (1 + ((center-D0[k]) / D0_center) ** (2 * n)) > 0.01):
                D0_center -= 0.1
            for i in range(center, len(Hk[0])):
                if (Hk[i, 0] > 0.9):
                    breedte = i - center
                    break
            Hk2[center - breedte:center + breedte, :] = 1 / (1 + (C / D0[k]) ** (2 * n))
            Hk[center - breedte:center + breedte, center - D0[k]:center + D0[k]] = Hk2[center - breedte:center + breedte,center - D0[k]:center + D0[k]]
...
for i in range(3):
    ...
    ax[i, 0].imshow(img);
    ax[i, 0].axis('off')
    ax[i, 1].imshow(np.log(1 + np.abs(F)));
    ax[i, 1].axis('off')
    ax[i, 2].imshow(np.log(1 + np.abs(G)));
    ax[i, 2].axis('off')
    ax[i, 3].imshow(g);
    ax[i, 3].axis('off')
plt.show()
