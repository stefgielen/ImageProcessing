...
def swap_faces(img1, img2=None, blendmode='pyramid', faceorder=(0, 1), display=True, flip_faces=(False, False)):
    ...
    if (img2 is None):
        if blendmode == 'alfa-blending':
            ...
            swapped = cv2.seamlessClone(warped1.astype('uint8'),img1.astype('uint8') ,mask2.astype('uint8')*255,center1,cv2.NORMAL_CLONE)
            swapped = cv2.seamlessClone(warped2.astype('uint8'), swapped.astype('uint8'), mask1.astype('uint8')*255, center2, cv2.NORMAL_CLONE)
            #plt.imshow(swapped)
            #plt.show()
